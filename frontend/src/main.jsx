...
async function apiFetch(path, options) {
  ...
  throw lastError || new Error('Backend nepasiekiamas')
}
...
function App() {
  ...
  const upload = async (e) => {
    ...
    try {
      const { res, base } = await apiFetch('/api/media/upload', { method: 'POST', body: fd })
      const data = await res.json()
      if (!res.ok) throw new Error(data.detail || 'Įkėlimas nepavyko')
      setStatus({ ok: `Statusas: ${data.status}`, proof: `${base}/api/proof/${data.proof_slug}` })
    } catch (err) {
      setStatus({ error: err.message })
    } finally {
      setLoading(false)
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-white">
      <div className="max-w-3xl mx-auto p-6">
        ...
        <p className="text-slate-300 mb-6">1) Registruokis / prisijunk → 2) Įkelk nuotrauką.</p>
        ...
        {active === 'register' && (
          <form onSubmit={register} className="grid gap-3 bg-slate-900 p-5 rounded-xl">
        ...
        {active === 'login' && (
          <form onSubmit={login} className="grid gap-3 bg-slate-900 p-5 rounded-xl">
        ...
        {active === 'upload' && (
          <form onSubmit={upload} className="grid gap-3 bg-slate-900 p-5 rounded-xl">
