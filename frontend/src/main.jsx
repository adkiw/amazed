  ...
  return (
    <main className="min-h-screen bg-slate-950 text-white"><div className="max-w-3xl mx-auto p-6">
      ...
      {active === 'upload' && <form onSubmit={upload} className="grid gap-3 bg-slate-900 p-5 rounded-xl">
        ...
        {previewUrl && mediaType === 'image' && <img src={previewUrl} alt="preview" className="max-h-52 rounded border border-slate-700" />}
      ...
      {stats && <div className="mt-5 bg-slate-900 border border-slate-700 rounded-xl p-4">
        <h3 className="font-semibold mb-2">Mano statistika</h3>
        <p>Viso įkėlimų: {stats.total_uploads}</p>
        <p>Nuotraukų: {stats.image_uploads} | Video: {stats.video_uploads}</p>
        <p className="mt-2 text-sm text-slate-300">Paskutiniai įkėlimai:</p>
        <ul className="text-sm list-disc pl-5 space-y-1">
          {stats.latest_uploads.map((u) => <li key={u.id}>
            #{u.id} {u.title} — {u.is_first_for_hash ? 'PIRMA tokia sistemoje' : `NE pirma, jūsų vieta: #${u.rank_for_same_hash}`} ; pirmas: {u.first_uploader_name || 'unknown'} (ID {u.first_uploader_id}) ; {u.first_registered_at}
          </li>)}
        </ul>
      </div>}
    </div></main>
